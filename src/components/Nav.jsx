...
const Nav = () => {
    ...
    return (
        <div className="absolute right-0 h-full w-full overflow-hidden">            
            {/* bars */}
            <div className='lg:w-24 lg:h-24 cursor-pointer z-10 absolute right-0 text-sand-50 text-3xl top-14'>
            ...
            <div className={`absolute z-20 bottom-full ${state.showMenu ? "translate-y-full opacity-100": "opacity-0"} w-full h-full bg-gray-900 flex justify-center items-center transition-all duration-1000 delay-75`}>
                ...
                {/* navbar title */}
                <h1 className={`absolute text-gray-500 flex justify-center items-center font-blackOps text-[160px] transition-all duration-1000 ${state.menuTitle ? "opacity-40 tracking-tighter":"opacity-0"}`}>{state.menuTitle}</h1>
                ...
                {/* exit icon */}
                <i className="fa-solid fa-door-closed cursor-pointer transition-all absolute top-24 right-24 duration-300 text-sand-50 text-3xl" 
                onMouseEnter={(e)=>{
